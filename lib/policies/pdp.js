...
Pdp.prototype.canWriteToAttribute = function (userInfo, entityInfo, attributeName, attributeValue) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var ps = [that.pap.getAttributePolicy(entityInfo.data.id, entityInfo.data.type),
    that.pap.getAttributePolicy(entityInfo.data.id, entityInfo.data.type, attributeName)];
    Promise.all(ps)
      .then(function(policies){
          ...
          return pdp.checkWrite(userInfo, policies[0],  entityInfo ,policies[1]);
...
Pdp.prototype.canWriteToAllAttributes = function (userInfo, entity, entity_id, entity_type, entity_owner) {
  var that = this;
  //clone and filter the /

  entity = clone(entity);
  entity.id = entity_id;
  entity.type = entity_type.substring(1);
  userInfo = clone(userInfo);
  userInfo.type = userInfo.type.substring(1);
  //promise that always resolves to collect all errors.
  function buildPromise(p){
    return new Promise(function (resolve, reject) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    for (var i in keys){
      promises.push(buildPromise(that.canWriteToAttribute({
        "type":"user",
        "data":userInfo
      }, {
        "type": entity_type.substring(1),
        "data":entity
      }, keys[i],
      entity[keys[i]],
      entity_owner)));

...
LevelStorage.prototype.init = function (storageConf, cb) {
  ...
  function createSomething(that, action_type, pk , owner, data){
    ...
    delete args[0];
    console.log(" internal args call createSomething "+JSON.stringify(args));
         ...
         if(error && error.notFound)
            ...
            handleError(error,reject);
             ...
             if(value){
               ...
               reject(createError(500,"unexpected empty response from get function in the database for pk "+pkToString(pk)));
             }
          }
      });
  ...
  return new Promise(function(resolve,reject){
    that.readGroupPromise(group_name, owner)
      .then(function(g){
        ...
        if(already_in_group.length > 0){
          return reject(createError(409, "entity with id "+pkToString(getPk("entities",entity))+" is already in group with group name "+group_name+ " owned by "+owner+" seems there is an inconsistency"));
        }
        else{

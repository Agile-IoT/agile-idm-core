        ...
        if (error) {
          ...
          for (var type in data) {
            if (typeof (data[type]) === "string"){
              that.store_string_attribute_value_statement.run(buildAttributePk(entity_pk, type), entity_pk, type, data[type],function(error){
                console.log("finished storing string attribute value with params "+JSON.stringify(arguments))
              });
...
SqliteStorage.prototype.createEntityPromise = function (entity_id, entity_type, owner, data) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    that.store_entity_statement.run(entity_pk, entity_id, entity_type, owner, function (error) {
      console.log('finished executing inster statement, error: '+error);
      if (error) {
        handleError(error, reject);
      } else {
    ...
    buildPromiseGetGroupById(that, group_id).then(function(group){
      ...
      that.get_memberships_by_grouppk.all(group_id, function (error, rows) {
        if (error) {
          ...
          var promisses = [];
          for (var i in rows) {
            console.log('sqlite3 processing row ' + JSON.stringify(rows[i]));
            promisses.push(buildPromiseEntityByPk(that,rows[i].fk_entity_pk));
          }
          Promise.all(promisses).then(function (entities) {
            console.log('sqlite3: finished processing listEntitiesByGroupId. resolving  with  ' + JSON.stringify(entities));
            return resolve(entities);
          },function rejection(re){
            reject(re);
          }).catch(function (reason) {

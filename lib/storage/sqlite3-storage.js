...
SqliteStorage.prototype.init = function (storageConf, onCreationFinished){
	...
	this.storage = new sqlite3.Database(filename,function(error){
			if(error){
				...
				with(this.storage){
					serialize(function(){
						run("CREATE TABLE IF NOT EXISTS Entity (id TEXT PRIMARY KEY, type TEXT NOT NULL)");
						...
						that.get_entity__id_by_string_attribute_value_statement = prepare("SELECT fk_entity_id from StringAttributeValue where  type = ? and value = ?");
						that.store_entity_statement = prepare("INSERT INTO Entity  (id , type) VALUES(?, ?)");
					});
		   }
...
SqliteStorage.prototype.listEntitiesByAttributeTypeAndValue = function(attribute_type, attribute_value, cb){
	var that = this;
	that.get_entity__id_by_string_attribute_value_statement.get(attribute_type, attribute_value, function (error,rows){
		if(error){ cb({"success":false,"error":error});}
    else if(!rows){ cb({"success":false,"error":"entity with attribute  "+attribute_type+" and value "+attribute_value+" not found"});}
		else{
	    var entities = [];
			var promisses = [];
			if(rows.lengh == 0){
			...
			for(var i in rows){
				 promisses.push(new Promise(function(resolve, reject) {
					 that.readEntity(rows[i], function(result){
		 				  if(result.success){
		 							entities.push(result.data);
									resolve();
		 					}
		 					else{
								  console.log("could not read entity with id: "+rows[i]+JSON.stringify(result));
		 							reject(new Error("could not read entity with id: "+rows[i]));
		 					}
		 			});
				 }));
				 Promise.all(promisses).then(function(){
					 return cb({"success":true, "data": entities});
				 }).catch(function(reason){
					 cb({"success":false,error:"a promise failed during the collection of entities in the query"});
				 });

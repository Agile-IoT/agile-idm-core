	...
	return new Promise(function(resolve,reject){
	    ...
	    if(error){ handleError(error, reject);}
	        ...
	        that.get_entity_attributes_by_id_statement.all(row.pk, function (error, rows){
			    if(error){ handleError(error, reject);}
			    else if(!rows || rows.length == 0){ reject(createError(404, "attributes for entity with id "+entity_id+"and type "+entity_type+" not found"));}
			    else{
...
SqliteStorage.prototype.listEntitiesByAttributeValueAndType = function(attribute_type, attribute_value){
	...
	return new Promise(function(resolve,reject){
		that.get_entity__id_by_string_attribute_value_statement.all(attribute_type, attribute_value, function (error,rows){
			if(error){ handleError(error, reject);}
	    else if(!rows || rows.length == 0){ reject(createError(404,"entity with attribute  "+attribute_type+" and value "+attribute_value+" not found"));}
			else{
		    ...
		    var promisses = [];
				...
				for(var i in rows){
					 ...
					 promisses.push(new Promise(function(res, rej) {
						 that.get_entity_by_pk_statement.get(rows[i]["fk_entity_pk"], function(error,result){
							 if(error){ handleError(error, reject);}
							 else if(!rows || rows.length == 0){ reject(createError(404,"entity with attribute  "+attribute_type+" and value "+attribute_value+" not found"));}
							 else{

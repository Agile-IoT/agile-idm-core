				...
				with(that.storage){
					serialize(function(){
					  ...
					  that.store_int_attribute_value_statement = prepare("INSERT INTO IntAttributeValue  (id, fk_entity_pk, type, value) VALUES(?, ?, ?, ?)");
						...
						that.get_entity_by_pk_statement = prepare("SELECT * from Entity WHERE pk =?");
...
SqliteStorage.prototype.readEntityPromise = function(entity_id, entity_type){
	...
	return new Promise(function(resolve,reject){
	    ...
	    if(error){ handleError(error, reject);}
...
SqliteStorage.prototype.listEntitiesByAttributeValueAndType = function(attribute_type, attribute_value){
	...
	return new Promise(function(resolve,reject){
		that.get_entity__id_by_string_attribute_value_statement.get(attribute_type, attribute_value, function (error,rows){
			if(error){ handleError(error, reject);}
	    else if(!rows){ reject(createError(404,"entity with attribute  "+attribute_type+" and value "+attribute_value+" not found"));}
			else{
		    ...
		    var entities = [];
				...
				for(var i in rows){
					 promisses.push(new Promise(function(res, rej) {
						 that.get_entity_by_pk_statement.get(rows[i], function(error,result){
							 if(error){ handleError(error, reject);}
							 else if(!rows){ reject(createError(404,"entity with attribute  "+attribute_type+" and value "+attribute_value+" not found"));}
							 else{
								  console.log('found '+JSON.stringify(result)+" as entity by attribute value and type");
					 ...
					 Promise.all(promisses).then(function(){
						 console.log('resolving attribute query with  '+JSON.stringify(entities));

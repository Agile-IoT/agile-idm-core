...
//helper function to build a JS object from sql row
function sqlRowsToObjecT(data){
  var result = {};
...
function sqlRowsToEntity(entity, attributes) {
  var result = sqlRowsToObjecT(attributes);
...
//converts the given sql-result rows to an object
function sqlRowsToGroup(group_name, owner, grouprow) {
    ...
    return new Promise(function (resolve, reject) {
      that.get_entity_by_id_and_type_statement.get(entity_id, entity_type, function (error, row) {
        if (error) {
          ...
          reject(createError(404, "entity with id " + entity_id + " and type " + entity_type + " not found"));
        } else {
          that.get_entity_attributes_by_id_statement.all(row.pk, function (error, rows) {
            if (error) {
              handleError(error, reject);
            } else if (!rows || rows.length === 0) {
              reject(createError(404, "attributes for entity with id " + entity_id + "and type " + entity_type + " not found"));
            } else {
...
SqliteStorage.prototype.readGroupPromise = function (group_name, owner) {
   console.log("arguments for readGroupPromise sqlite3 " + JSON.stringify(arguments));
    var that = this;
    return new Promise(function (resolve, reject) {
      console.log('executing sqlite3 read group promise');
      var pk = buildGroupPk(owner,group_name);
      console.log('pk'+pk);
      console.log('that.get_group_by_id_statement.'+that.get_group_by_id_statement);
      that.get_group_by_id_statement.get(pk, function (error, row) {
        console.log('that.get_group_by_id_statement.'+JSON.stringify(error));
        console.log('that.get_group_by_id_statement.'+JSON.stringify(row));
        if (error) {
          handleError(error, reject);
        } else if (!row || row.length === 0) {
          reject(createError(404, "group with id " + pk + " not found"));
        } else {
          var res = sqlRowsToGroup(group_name, owner, row);
...
SqliteStorage.prototype.listEntitiesByAttributeValueAndType = function (attribute_type, attribute_value) {
  ...
  function buildPromiseEntityByPkAcc(that,row){
    return new Promise(function (res, rej) {
      that.get_entity_by_pk_statement.get(row.fk_entity_pk, function (error, result) {
        if (error) {
          handleError(error, rej);
        } else if (!result || result.length === 0) {
          rej(createError(404, "entity with attribute  " + attribute_type + " and value " + attribute_value + " not found"));
        } else {
          console.log('entity found ' + JSON.stringify(result) + " as entity by attribute value and type");
          that.readEntityPromise(result.entity_id, result.entity_type).then(function (r) {
            res(r);
          });
        }
      });
    ...
    that.get_entity__id_by_string_attribute_value_statement.all(attribute_type, attribute_value, function (error, rows) {
      if (error) {
        ...
        for (var i in rows) {
          ...
          promisses.push(buildPromiseEntityByPkAcc(that,rows[i]));
        }
